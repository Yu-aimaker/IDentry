...
export default function DashboardPage() {
  ...
  const [user] = useState({ name: '山田太郎', email: 'yamada@example.com' });
  const router = useRouter();

  useEffect(() => {
    // 実際のアプリでは、APIからユーザーのプロフィール一覧を取得
    const mockProfiles: Profile[] = [
      {
        id: '1',
        name: '山田太郎のポートフォリオ',
        bio: 'フロントエンドエンジニアとして3年の経験があります。',
        profileUrl: 'https://identry.com/p/yamada-taro',
        isPublic: true,
        views: 156,
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-black">
                  {profiles.reduce((total, profile) => total + profile.views, 0)}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-xl border border-gray-100">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-black">
                  {profiles.filter(profile => profile.isPublic).length}
                </p>
              </div>
            </div>
          </div>
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
                    <div className="flex-1 mb-4 lg:mb-0">
                      ...
                      <div className="flex items-center space-x-4 text-sm text-gray-500">
                        <span>👁️ {profile.views} 回閲覧</span>
                        <span>📅 作成日: {profile.createdAt}</span>
                        <span>✏️ 更新日: {profile.lastModified}</span>
                      </div>
                    </div>
          ...
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full">
            <div className="text-center">
              ...
              <div className="space-y-3">
                <button
                  onClick={() => copyToClipboard(selectedProfile.profileUrl)}
